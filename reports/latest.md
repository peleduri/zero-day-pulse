...
1. **Authorization revocation bypas…
...
Poweradmin v4.3.2 uses the attacker-controlled `HTTP_HOST` request header as the
  ...
  victim&#x27;s authorization code to an attacker-control…
...
raw `Location` header, follows the redirect recursively, and …
...
The `CompletionsHandler` function in `pkg/github/server.go:198` accesses `params.Ref` without checking if it&#x27;s nil …
