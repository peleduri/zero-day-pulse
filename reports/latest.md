...
> ### Summary
The **GET `/api/v1/upsert-history`** endpoint returns the **entire server-wide upsert history** (response size **&gt;100MB**) instead of being scoped to the requesting user/tenant/workspace. The response includes **sensitive configuration data** (e.g., Vector Store settings such as **Qdrant Server URL** and **collection name**), resulting in a **High severity information disclosure** t…
...
Route permissions (`assistants:*`) only check feature access. They do not…
...
> ## Finding — Unauthorized Workspace Variables disclosure via $vars injection (bypasses variables:view)
  ...
  - Runtime variables are resolved from server environment variables: flowise-src/packages/components…
...
> # summary:
In Flowise, `DELETE /api/v1/chatflows/:id` authorizes requests with `checkAnyPermission(&#x27;chatflows:delete,agentflows:delete&#x27;)`. Possession of either permission is sufficient to reach the delete path. The delete logic does not validate the target resource `type`, allowing a caller with only `agentflows:delete` to delete a `CHATFLOW`, and a caller with only `chatflows:delete` to…
...
3. `process.mainModule.constructor._load(&#x…
